...
\subsection{Model of Clustering and Channel Selection}
We consider the performance of BSSs in a typical cluster. Users are located on a 2-D plane following a HPPP with density $\lambda$. The cluster size is $K$ and there are $M$ channels. The cluster head of the typical cluster is located at the origin, with $K-1$ cluster members randomly distributed in the disc centered at the origin. The radius of the disc, $R_{\mathrm{cluster}}$, satisfies that 
\begin{align*}
...
We only consider the LOS channel between PCPs in our analysis. The channel between any pair of PCPs are independent and the state of the channel is determined by the length of the channel, i.e. at any time $t$, a channel of length $d<r_{\max}$ is LOS channel with probability $\mathrm{P_{LOS}}(d)$. 

In each BSS, there are three types of transmissions with different directionality and transmit power as follows,
\begin{itemize}
...
The antenna gain of directional antennas is modeled using a sector antenna model, see Fig. \ref{fig:clusteringanalysis:antenna}. The width of the main lobe is $\theta_t$ and $\theta_r$, for transmit antenna and receive antenna respectively. The antenna gain in the main lobe is $M_t$ and $M_r$ and the gain out side the main lobe is $m_t$ and $m_r$.

The transceiver of one link point their beams to each other while other transmitters points their beams to a random direction uniformly distributed in $[0, 2\pi]$. The power receiver $i$ receives from transmitter $j$ is given as follows, 
\begin{equation*}
...
PL(i,j) = 
\begin{cases}
\big(\frac{4\pi d(i,j) freq}{c}\big)^2, & w.p. ~ \mathrm{P_{LOS}}(d(i,j))\\
0, & w.p. ~ 1-\mathrm{P_{LOS}}(d(i,j))
\end{cases}.
\end{equation*}
...
Notice that the transmit power, beam width and beam gain are related to the transmission type of $j$, which is influenced by the scheduling, clustering, as well as the traffic patterns of users.

After describing the channel model, we now introduce the model for MAC. Denote $T_{\mathrm{frame}}$ as the length of frame and $T_{\mathrm{beacon}}$ as the length of one Beacon SP. The cluster head reserves exact $K$ Beacon SPs thus $K\cdot T_{\mathrm{beacon}}$ is reserved for beacon transmission in each frame and $T_{\mathrm{data}} = T_{\mathrm{frame}} - K\cdot T_{\mathrm{beacon}}$ is used for data transmission. The proportion of data reserved for data transmission is given by
\begin{equation*}
p_{\mathrm{data}} = \frac{T_{\mathrm{data}}}{T_{\mathrm{frame}}}.
\end{equation*}
In each Beacon SP, the PCP of exactly one BSS of the cluster will transmit its beacon and the other devices will receive the beacon using omni-directional mode. 

We consider two types of scheduling method within the cluster, TDMA and ideal resource reuse (IRR). In TDMA scheduling, the fraction of that a BSS can access the channel in a frame is given by, 
\begin{equation*}
p_{\mathrm{access}}^{\mathrm{TDMA}} = p_{\mathrm{data}}/K.
\end{equation*}
In IRR, we assume there is ideal resources sharing mechanism among the BSSs in the cluster, i.e., independent set scheduling [ref required]. Let $N_{\mathrm{intra-cluster}}$ be the number of other BSSs in the same cluster that interferes with the data transmission of BSS, then the probability that the BSS can access the channel can be approximated by, 
\begin{equation*}
...
A BSS's transmission is successful if there is no strong interference from inter-cluster interference. Denote $N_{\mathrm{inter-cluster}}$ as the number of BSSs outside the the cluster. The probability of successful transmission is approximated as follows, 
\begin{equation*}
p_{\mathrm{success}} = e^{-\E[N_{\mathrm{inter-cluster}}]}.
\end{equation*} 

Based on the above analysis, we can evaluate the throughput of a BSS using the average successful transmission time (STT) of the user,
\begin{equation*}
STT = p_{\mathrm{data}}\times p_{\mathrm{access}} \times p_{\mathrm{success}}.
\end{equation*}

\subsection{Numerical Results and Discussion}
In this section, we compute $STT$ for different scenarios and characterize the optimal cluster size. 
Some common parameters are listed in Table \ref{tab:clusteranalysis:parameter}.
