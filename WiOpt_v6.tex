...
It is an open question if and how the MAC protocols should adapt to work in such environments.%, e.g., how much overhead is required to coordinate potential interferers and handle user dynamics.
...
%Furthermore, the ``worst case'', e.g., where users see highest number of interferers and most sensitivity to motion, does not necessarily correspond to scenarios with the highest user density due to blockage.
...
\begin{itemize}
	...
	\item We show that the \emph{per user throughput} first decreases with user density as expected, but then remains stable and even increases at high densities. The optimal cluster size also remains roughly constant in dense scenarios. Our results suggest that, perhaps surprisingly, the MAC of wearable networks is ``scalable'' to high user densities. %The resources allocated to each user may not decrease with density, thus the total reuse of the network may scale with user density.
\end{itemize}

%Our results suggest that MAC schedulers may be optimized to device transmission capabilities and QoS requirements. 
%More importantly, for high density scenarios, the overheads of MAC and per user throughput may remain the same, thus the MAC can be well scalable.
...
There is no access point (AP) or central controller to coordinate or synchronize transmission across all users. % with the exclusion of blocking (discussed later)??. 
...
\begin{figure}
	\centering
	\includegraphics[width = 0.25\textwidth]{Channel_selfblockage.pdf}
...
Consider the channel between the typical user at the origin and the user at location $x$, and a potential blocking user $(x', \theta')$, see Fig.~\ref{fig:channel:blockage}\subref{subfig:channel:blockage:LOS}.
...
For NLOS channels, $N_{\mathrm{B}}^\mathrm{NLOS}(x)$ also follows Poisson distribution and its mean can be computed similarly.% as $N_{\mathrm{B}}^\mathrm{LOS}(x)$ .
...
\begin{figure}
	\centering
	\includegraphics[width = 0.3\textwidth]{Channel_en_si_wiopt.pdf}
...
%As a result, 
%\begin{gather*}
%\sigma_{Y_x^t}^2 = \sigma_{Y_x^{t+\Delta t}}^2  = \mathrm{P}_{\mathrm{SI}}(x)\cdot %\E[Y_x^{t+\Delta t}] = \E[Y_x^{t}] = \mathrm{P_{SI}}(x),
%\end{gather*}
...
%$\Delta X$ is uniformly distributed in a disc centered at the origin with the radius of $r(\lambda, \Delta t)$, $\Delta X \sim \mathrm{unif}(b(0, r(\lambda, \Delta t)))$. 
...
Fig.~\ref{fig:channel:sensitivity}\subref{subfig:channel:sensitivity:average} exhibits the sensitivity of a typical strong interferer, $\E[S]$,
\begin{equation*}
...
\begin{figure}
	\centering
	\subfloat[Sensitivity of user $x$]{\label{subfig:channel:sensitivity:function}\includegraphics[width=0.23\textwidth]{Channel_sensitivity_wiopt16.pdf} } \hfill
	\subfloat[Average sensitivity]{\label{subfig:channel:sensitivity:average} \includegraphics[width=0.23\textwidth]{Channel_sensitivity_average_wiopt16.pdf}}
	
	\caption[]{Sensitivity of users at different distance $|x|$ \subref{subfig:channel:sensitivity:function}, and average sensitivity of a typical strong interferer $\E[S]$ for different user densities \subref{subfig:channel:sensitivity:average}.}
...
\begin{figure}
	\centering
	\includegraphics[width = 0.4\textwidth]{Clustering_reuse_wiopt.pdf}
...
%study how cluster size influences the resources made available to each PBSS in a dense wearable networks where clustering and hierarchical scheduling are used. 
...
%The clusters can be ignored from the point of view of the inter-cluster interference seen by a user. 
%For users out side the cluster, we assume they are uniformly distributed outside the protection area with density $\lambda$.
%These users are not synchronized with the typical cluster and work independently.
%The probability that a user work on the same channel with the typical cluster is $1/M$.
...
%The STT is the average over different users. 
%In HRR, in each slot, non-priority users are given a random priority index and users contend for the access to channel based on their priority index. 
%Simulation is performed over different network topologies.
...
\begin{figure}[htp]
	\centering
	\subfloat[STT in TDMA ]{\label{subfig:cluster:validation:tdma}\includegraphics[width=0.23\textwidth]{Cluster_analysis_validation_tdma.pdf} } \hfill
	\subfloat[STT in HRR]{\label{subfig:cluster:validation:hrr} \includegraphics[width=0.23\textwidth]{Cluster_analysis_validation_hrr.pdf}}
	
	\caption[]{Total STT (T.), STT of primary transmissions (P.) and STT of secondary transmissions (S.) in TDMA %\subref{subfig:cluster:validation:tdma} 
		and HRR %\subref{subfig:cluster:validation:hrr}
...
\begin{figure}
	\centering
	\includegraphics[width = 0.3\textwidth]{Cluster_analysis_directionality_wiopt.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width = 0.3\textwidth]{Cluster_analysis_optimal_cluster_size_wiopt.pdf}
...
%Such results indicate that the MAC for dense wearable network scales with user density, and the design of MAC should focus more on intermediate dense environments. 
...
\begin{figure}
	\centering
	\includegraphics[width = 0.3\textwidth]{Cluster_analysis_compare_mac_wiopt.pdf}
...
An ideal cluster protocol should be able to adapt to user transmission capabilities and QoS requirements. %  but choice of optimal cluster size may be robust to user density.
